...
<Select.Root type="single" bind:value items={themes}>
	<Select.Trigger
		class="inline-flex h-[27px] w-full select-none items-center rounded-[5px] border border-border-input bg-background px-2 pr-1 text-[8px] transition-colors placeholder:text-foreground-alt/50 dark:border-[#18181B2B] dark:bg-white dark:text-[#171717] lg:h-[37px] lg:rounded-[9px] lg:px-3 lg:pr-2 lg:text-sm"
		aria-label="Select task"
	>
		{selectedLabel}
		<CaretUpDown class="ml-auto size-3 text-muted-foreground dark:text-[#17171766] lg:size-6" />
	</Select.Trigger>
	<Select.Portal>
		<Select.Content
