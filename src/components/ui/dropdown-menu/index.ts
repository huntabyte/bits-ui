...
type SubContentTransitionParams = {
	x: number;
	start: number;
	duration: number;
};

export const subContentTransition = (
	node: Element,
	params: SubContentTransitionParams = { x: -5, start: 0.95, duration: 200 }
): TransitionConfig => {
	const style = getComputedStyle(node);
	const transform = style.transform === "none" ? "" : style.transform;
	...
	return {
		...
		delay: 0,
		css: (t) => {
			const x = scaleConversion(t, [0, 1], [params.x, 0]);
			const scale = scaleConversion(t, [0, 1], [params.start, 1]);

			return styleToString({
				transform: `${transform} translate3d(${x}px, 0, 0) scale(${scale})`,
				opacity: t
			});
		},
		easing: cubicOut
