...
<DateRangeField.Root bind:value>
	<div class="flex w-full max-w-[280px] flex-col gap-1.5">
		...
		<DateRangeField.Input
			let:segments
			class="flex h-input w-full max-w-[280px] select-none items-center rounded-input border border-border-input bg-background px-2 py-3 text-sm tracking-[0.01em] text-foreground focus-within:border-border-input-hover focus-within:shadow-date-field-focus hover:border-border-input-hover"
		>
			{#each segments.start as { part, value }}
				<div class="inline-block select-none">
					{#if part === "literal"}
						<DateRangeField.Segment type="start" {part} class="p-1 text-muted-foreground">
							{value}
			...
			{#each segments.end as { part, value }}
				<div class="inline-block select-none">
					{#if part === "literal"}
						<DateRangeField.Segment type="end" {part} class="p-1 text-muted-foreground ">
							{value}
						</DateRangeField.Segment>
					{:else}
						<DateRangeField.Segment
