...
describe("Menubar", () => {
	...
	it("has bits data attrs", async () => {
		...
		expect(root).toHaveAttribute("data-b-menubar-root");
		...
		for (const part in parts) {
			...
			expect(el).toHaveAttribute(`data-b-menu-${parts[part]}`);
		}

		await user.click(getByTestId("1-sub-trigger"));

		const subContent = getByTestId("1-sub-content");
		expect(subContent).toHaveAttribute(`data-b-menu-sub-content`);
	});
