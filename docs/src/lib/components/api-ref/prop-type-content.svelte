		...
		<Tooltip.Root>
			...
			<Tooltip.Portal>
				<Tooltip.Content side="top" sideOffset={8}>
					<div
						class="rounded-input border-dark-10 bg-background shadow-popover flex items-center justify-center border p-3 text-sm font-medium outline-hidden"
					>
						{tooltipContent}
					</div>
				</Tooltip.Content>
...
{#snippet TypeContent({ typeDef }: { typeDef: string | Component })}
	{#if typeof typeDef === "string"}
		<Popover.Content
			...
			side="top"
			sideOffset={10}
			class="focus-override rounded-card border-border bg-background shadow-popover z-50 border p-4 outline-hidden"
		>
	...
	{:else}
		...
		>
			...
			<div
				class="**:data-line:pr-2.5! [&_pre]:my-0! [&_pre]:mb-0! [&_pre]:mt-0 [&_pre]:overflow-x-visible! [&_pre]:border-0 [&_pre]:p-0 [&_pre]:pb-0! [&_pre]:pt-0! [&_pre]:outline-hidden! [&_pre]:ring-0! [&_pre]:ring-offset-0!"
			>
				<TypeDef />
			</div>
		</Popover.Content>
