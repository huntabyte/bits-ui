...
<Select.Root type="single" bind:value items={themes}>
	...
	<Select.Portal>
		<Select.Content
			class="focus-override border-muted bg-background shadow-popover z-50 max-h-96 w-[var(--bits-select-anchor-width)] min-w-[var(--bits-select-anchor-width)] select-none rounded-xl border px-1 py-2 outline-hidden"
			sideOffset={10}
		>
