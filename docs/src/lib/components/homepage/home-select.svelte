...
<Select.Root type="single" bind:value items={themes}>
	<Select.Trigger
		class="cursor-pointer border-border-input bg-background placeholder:text-foreground-alt/50 inline-flex h-[27px] w-full select-none items-center rounded-[5px] border px-2 pr-1 text-[8px] transition-colors lg:h-[37px] lg:rounded-[9px] lg:px-3 lg:pr-2 lg:text-sm dark:border-[#18181B2B] dark:bg-white dark:text-[#171717]"
		aria-label="Select task"
	>
		{selectedLabel}
		<CaretUpDown class="text-muted-foreground ml-auto size-3 lg:size-6 dark:text-[#17171766] mr-[-5px]" />
	</Select.Trigger>
	<Select.Portal>
		<Select.Content
