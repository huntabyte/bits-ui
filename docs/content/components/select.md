...
```svelte /position="item-aligned"/ /preventScroll/
<Select.Content position="item-aligned" preventScroll>
	<Select.ScrollUpButton />
	<Select.Viewport>
		<Select.Item />
	</Select.Viewport>
...
Radix Select locks page scrolling while its content is open. Bits UI keeps scroll locking opt-in, so pass `preventScroll` as shown above when you want the same behavior.
