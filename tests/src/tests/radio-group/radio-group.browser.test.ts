...
async function setup(props: Partial<RadioGroupTestProps> = {}, items: Item[] = TEST_ITEMS) {
	await render(RadioGroupTest, { ...props, items });
...
describe("Value Changes", () => {
	it("should change the value when an item is clicked", async () => {
		...
		for (const indicator of INDICATOR_IDS) {
			await expect.element(page.getByTestId(indicator)).toMatchTextContent("false");
		}
	...
	it("should not change the value when a disabled item is clicked", async () => {
		await setup({}, [...TEST_ITEMS, { value: "e", disabled: true }]);

		const item = page.getByTestId("e-item");
		await item.click({ force: true });
		await expect.element(page.getByTestId("e-indicator")).toMatchTextContent("false");
	});
	...
	it("should not change the value when an unselected item receives focus without keyboard navigation", async () => {
		...
		await expect.element(page.getByTestId("b-indicator")).toMatchTextContent("true");
		await expect.element(page.getByTestId("a-indicator")).toMatchTextContent("false");
	});
	...
	it("should respect the value prop & binding", async () => {
		...
		expect(binding).toMatchTextContent("b");
		const bindingIndic = page.getByTestId("b-indicator");
		expect(bindingIndic).toMatchTextContent("true");
		await binding.click();
		await expect.element(binding).toMatchTextContent("a");
		await expect.element(page.getByTestId("b-indicator")).toMatchTextContent("false");
		const bindingIndic2 = page.getByTestId("a-indicator");
		expect(bindingIndic2).toMatchTextContent("true");
	});
});
...
describe("Input Behavior", () => {
	...
	it("should render an input if the `name` prop is passed", async () => {
		const t = await setup({ name: "radio-group" });
	...
	it("should make the input required if the radio group is required", async () => {
		const t = await setup({ required: true, name: "radio-group" });
	...
	it("should make the input disabled if the radio group is disabled", async () => {
		const t = await setup({ disabled: true, name: "radio-group" });
...
describe("Readonly Behavior", () => {
	...
	it("should not change value when readonly and item is clicked", async () => {
		await setup({ readonly: true, value: "b" });
...
describe("Focus Management", () => {
	...
	it("should not change value when a value is set and an onOpenAutoFocus occurs", async () => {
		await render(RadioGroupPopoverTest, {
			value: "b",
			items: TEST_ITEMS,
		});
		...
		await expect.element(page.getByTestId("value")).toMatchTextContent("b");
...
describe("Scroll Behavior", () => {
	it("should not make the page scrollable when rendering a named input in nested overflow containers", async () => {
		const t = await render(RadioGroupScrollLayoutTest, { name: "radio-group" });
