...
async function setupMultiple(props: Partial<MultipleToggleGroupTestProps> = {}) {
	// @ts-expect-error - shh
	await render(ToggleGroupMultipleTest, { ...props, items });
...
async function setup(props: Partial<SingleToggleGroupTestProps> = {}) {
	// @ts-expect-error - shh
	await render(ToggleGroupTest, { ...props, items });
	const root = page.getByTestId("root");
	const binding = page.getByTestId("binding");
	return {
		root,
...
describe("Toggle Group", () => {
	...
	it("should toggle when clicked", async () => {
		...
		const item = page.getByTestId("item-1");
		await item.click();
		await expect.element(t.binding).toMatchTextContent("1");
		const item2 = page.getByTestId("item-2");
		await item2.click();
		await expect.element(t.binding).toMatchTextContent("2");
	});
	...
	it("should respect binding to the `value` prop", async () => {
		await setup();
		const binding = page.getByTestId("binding");
		await expect.element(binding).toMatchTextContent("");
		await binding.click();
		await expect.element(binding).toMatchTextContent("4");
		const item4 = page.getByTestId("item-4");
	...
	it("should allow multiple items to be selected when `kind` is `'multiple'`", async () => {
		...
		await item1.click();
		await item2.click();
		await expect.element(binding).toMatchTextContent("1,2");
		await item3.click();
		await expect.element(binding).toMatchTextContent("1,2,3");
		await item4.click();
		await expect.element(binding).toMatchTextContent("1,2,3,4");
		await item1.click();
		await expect.element(binding).toMatchTextContent("2,3,4");
		await item2.click();
		await expect.element(binding).toMatchTextContent("3,4");
		await item3.click();
		await expect.element(binding).toMatchTextContent("4");
		await item4.click();
		await expect.element(binding).toMatchTextContent("");
